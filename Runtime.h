...
inline Vec3 CoefficientES22(SuperCell *superCell, rMesh *tar, int x, int y, int z) {
	Vec3 Ans(0, 0, 0);
	Ans = Add(Ans, tar->Dots[x * superCell->b + y]);
	Ans = Add(Ans, tar->Dots[((x + 1) >= superCell->a ? 0 : x + 1) * superCell->b + y]);
	Ans = Add(Ans, tar->Dots[x * superCell->b + ((y + 1) >= superCell->b ? 0 : y + 1)]);
	Ans = Add(Ans, tar->Dots[((x + 1) >= superCell->a ? 0 : x + 1) * superCell->b + 
			((y + 1) >= superCell->b ? 0 : y + 1)]);
	return VAbs(Ans);
}

inline Vec3 CoefficientES22_Upd(SuperCell *superCell, rMesh *tar, int x, int y, int z, Vec3 S) {
	Vec3 Ans(0, 0, 0);
	Vec3 Rec = tar->Dots[x * superCell->b + y];
	tar->Dots[x * superCell->b + y] = S;
	Ans = Add(Ans, CoefficientES22(superCell, tar, x, y, z));
	Ans = Add(Ans, CoefficientES22(superCell, tar, x == 0 ? superCell->a - 1 : x - 1, y, z));
	Ans = Add(Ans, CoefficientES22(superCell, tar, x, y == 0 ? superCell->b - 1 : y - 1, z));
	Ans = Add(Ans, CoefficientES22(superCell, tar, x == 0 ? superCell->a - 1 : x - 1,
			y == 0 ? superCell->b - 1 : y - 1, z));
	tar->Dots[x * superCell->b + y] = Rec;
	Ans = Add(Ans, Rev(CoefficientES22(superCell, tar, x, y, z)));
	Ans = Add(Ans, Rev(CoefficientES22(superCell, tar, x == 0 ? superCell->a - 1 : x - 1, y, z)));
	Ans = Add(Ans, Rev(CoefficientES22(superCell, tar, x, y == 0 ? superCell->b - 1 : y - 1, z)));
	Ans = Add(Ans, Rev(CoefficientES22(superCell, tar, x == 0 ? superCell->a - 1 : x - 1,
			y == 0 ? superCell->b - 1 : y - 1, z)));
	return Ans;
}
...
rMesh* InitRMesh(SuperCell *superCell, Vec3 Field, double T, int Model, void (*GetEnergy)(rMesh*, SuperCell*)) {
	rMesh* self = NULL;
					...
					switch (superCell->Type) {
						case ModelM :
							self->Mag = Add(self->Mag, 
								Mul(self->Dots[((i * superCell->b + j) * superCell->c + k) * superCell->unitCell.N + l], 
								CoefficientM(i, j, k)));
							break;
						case ModelEC42AFE :
							self->Mag = Add(self->Mag, 
								Mul(self->Dots[((i * superCell->b + j) * superCell->c + k) * superCell->unitCell.N + l], 
								CoefficientEC42AFE(i, j, k)));
							break;
						case ModelEP22AFE :
							self->Mag = Add(self->Mag, 
								Mul(self->Dots[((i * superCell->b + j) * superCell->c + k) * superCell->unitCell.N + l], 
								CoefficientEP22AFE(i, j, k)));
							break;
						case ModelEP21FE :
							self->Mag = Add(self->Mag, 
								Mul(self->Dots[((i * superCell->b + j) * superCell->c + k) * superCell->unitCell.N + l], 
								CoefficientEP21FE(i, j, k)));
							break;
						case ModelES22 :
							self->Mag = Add(self->Mag, CoefficientES22(superCell, self, i, j, k));
							break;
						default:
							break;
					}
